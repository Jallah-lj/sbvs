...
$stmt = $db->prepare(
    "SELECT t.id AS db_id, t.teacher_id, t.phone, t.specialization, t.status, t.photo_url,
...
if ($role === 'Teacher' && (int)$teacher['user_id'] !== $sessionUserId) {
    header("Location: teachers.php");
    exit;
}
...
if (!empty($teacher['photo_url'])) {
    $relativePhoto = ltrim((string)$teacher['photo_url'], '/');
    $photoAbs = dirname(__DIR__, 3) . '/' . $relativePhoto;
    if (is_file($photoAbs)) {
        $teacherPhoto = rtrim(defined('BASE_URL') ? BASE_URL : '/', '/') . '/' . $relativePhoto;
    }
}

// ── Assigned classes (via branch + teacher's specialization linkage) ──────────
// Classes in the same branch as the teacher
$bStmt = $db->prepare(
            ...
            <div class="col-md-4 col-lg-3">
                ...
                <div class="card shadow-sm border-0 mb-4 pb-2">
                    <div class="card-body text-center pt-5 pb-4 px-4">
                        <?php if ($teacherPhoto): ?>
                        <div class="mx-auto mb-3 position-relative" style="width:96px; height:96px;">
                            <img src="<?= htmlspecialchars($teacherPhoto) ?>" alt="Instructor photo" class="rounded-circle border" style="width:96px; height:96px; object-fit:cover;">
                ...
                <div class="d-flex flex-column gap-3 mb-4">
                    <div class="stat-pill shadow-sm">
                        <div class="title">Classes in Branch</div>
                ...
                <div class="card shadow-sm border-0">
                        ...
                        <ul class="nav nav-tabs card-header-tabs" style="transform: translateY(1px);">
                            ...
                            <li class="nav-item">
                                <button class="nav-link py-3" data-bs-toggle="tab" data-bs-target="#tab-batches">
                                    <i class="bi bi-collection-fill me-2"></i>Assigned Classes
                    ...
                    <div class="card-body tab-content p-3 p-md-4">
                        ...
                        <div class="tab-pane fade" id="tab-batches">
                            <h6 class="fw-bold text-dark mb-4" style="letter-spacing: -0.01em;"><i class="bi bi-collection me-2 text-primary"></i>Branch Classes</h6>
                            <?php if (empty($batches)): ?>
                                <div class="alert alert-primary bg-primary bg-opacity-10 border-0 text-primary py-3 rounded-3 d-flex align-items-center">
                                    ...
                                    <div>No classes currently in this instructor's branch.</div>
                                </div>
                            <?php else: ?>
                            <div class="table-responsive">
                                <table class="table table-hover align-middle w-100 mb-0">
                                    <thead class="table-light text-muted small text-uppercase" style="letter-spacing: 0.05em;">
                                        <tr>
                                            <th class="ps-3 fw-semibold py-3 border-bottom-0">Class Name</th>
