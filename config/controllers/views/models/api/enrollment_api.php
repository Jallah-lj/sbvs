...
switch ($action) {
    ...
    // ── Get available courses for student's branch ───────────────────────────
    case 'get_courses':
        $sid = (int)($_GET['student_id'] ?? 0);
    ...
    // ── Get classes for a given course (compat endpoint) ─────────────────────
    case 'get_batches':
    ...
    // ── Enroll student in a course (class resolved internally) ───────────────
    case 'enroll':
        $sid    = (int)($_POST['student_id']     ?? 0);
        $cid    = (int)($_POST['course_id']      ?? 0);
        $bid    = (int)($_POST['batch_id']       ?? 0); // optional backward compatibility
        $eDate  = trim($_POST['enrollment_date'] ?? date('Y-m-d'));
        $status = trim($_POST['status']          ?? 'Active');

        if (!$sid || !$cid) {
            echo json_encode(['success' => false, 'message' => 'Student and course are required.']); break;
        }
        ...
        // Resolve internal class row for compatibility with existing schema.
        if ($bid > 0) {
            $bc = $db->prepare("SELECT id FROM batches WHERE id=? AND course_id=?");
            $bc->execute([$bid, $cid]);
            if (!$bc->fetch()) {
                echo json_encode(['success' => false, 'message' => 'Selected class does not belong to the chosen course.']); break;
            }
        } else {
            $latest = $db->prepare("SELECT id FROM batches WHERE course_id = ? ORDER BY start_date DESC, id DESC LIMIT 1");
            $latest->execute([$cid]);
            $bid = (int)$latest->fetchColumn();

            if ($bid <= 0) {
                $courseRow = $db->prepare("SELECT branch_id, name FROM courses WHERE id = ? LIMIT 1");
                $courseRow->execute([$cid]);
                $courseInfo = $courseRow->fetch(PDO::FETCH_ASSOC);
                if (!$courseInfo) {
                    echo json_encode(['success' => false, 'message' => 'Course not found.']); break;
                }

                $className = 'Course Class - ' . date('M Y');
                $startDate = $eDate ?: date('Y-m-d');
                $endDate   = date('Y-m-d', strtotime($startDate . ' +6 months'));
                $insBatch = $db->prepare(
                    "INSERT INTO batches (branch_id, course_id, name, start_date, end_date, status)
                     VALUES (?, ?, ?, ?, ?, 'Active')"
                );
                $insBatch->execute([(int)$courseInfo['branch_id'], $cid, $className, $startDate, $endDate]);
