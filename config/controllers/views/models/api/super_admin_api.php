...
if ($action == 'save' && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // ========================================================
    // STRICT PROTOCOL: Verify Authorizing Agent's Master Key
    // ========================================================
    $authSignatureBase64 = $_POST['authorization_signature'] ?? '';
    if (empty($authSignatureBase64)) {
        echo json_encode(["status" => "error", "message" => "Authorization Failed: Digital signature required."]);
        exit;
    }

    $authorizingPassword = base64_decode($authSignatureBase64);
    if ($authorizingPassword === false) {
        echo json_encode(["status" => "error", "message" => "Authorization Failed: Invalid signature format."]);
        exit;
    }

    // Fetch the currently logged-in Super Admin's password to verify
    $stmtAuth = $db->prepare("SELECT password_hash FROM users WHERE id = ?");
    $stmtAuth->execute([$_SESSION['user_id']]);
    $currentUser = $stmtAuth->fetch(PDO::FETCH_ASSOC);

    if (!$currentUser || !password_verify($authorizingPassword, $currentUser['password_hash'])) {
        echo json_encode(["status" => "error", "message" => "AUTHORIZATION FAILED: The master key provided is invalid."]);
        exit;
    }
    // ========================================================
