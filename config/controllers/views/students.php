        ...
        <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 mt-2 gap-3 fade-in">
            <div>
                ...
                <div class="d-flex align-items-center gap-2 mt-1">
                    <p class="text-muted small mb-0">Manage student records, enrollments, and profiles</p>
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="enroll_batch" class="form-select" disabled>
...
<script>
const IS_SA_OR_BA = <?= ($isSuperAdmin || ($role === 'Branch Admin')) ? 'true' : 'false' ?>;
const CAN_EDIT    = <?= ($isSuperAdmin || in_array($role, ['Branch Admin', 'Admin'])) ? 'true' : 'false' ?>;
$(document).ready(function () {
    ...
    const table = $('#studentTable').DataTable({
        processing: true,
        ajax: {
            url: '../views/models/api/student_api.php?action=list',
    ...
    function loadQuickAddCourses(branchId) {
        ...
        $('#enroll_batch').html('<option value="">-- Select Course First --</option>').prop('disabled', true);
    ...
    function resetQuickAddModal() {
        ...
        $('#enroll_batch').val('').html('<option value="">-- Select Course First --</option>').prop('disabled', true);
    ...
    // Course change → load batches + show fee
    $('#enroll_course').on('change', function() {
        const cid  = $(this).val();
        const fee  = parseFloat($(this).find(':selected').data('fee') || 0);
        const $bat = $('#enroll_batch');
        if (!cid) {
            $bat.html('<option value="">-- Select Course First --</option>').prop('disabled', true);
            $('#quickPaymentSection').hide();
            $('#courseFeeInfo').hide();
            return;
        }
        ...
        $bat.html('<option value="">Loading batches...</option>').prop('disabled', true);
        $.getJSON('../views/models/api/enrollment_api.php?action=get_batches&course_id=' + cid, function(res) {
            if (res.success && res.data.length) {
                let opts = '<option value="">-- Select Batch --</option>';
                res.data.forEach(b => {
                    opts += `<option value="${b.id}">${b.name} (${b.start_date} → ${b.end_date})</option>`;
                });
                $bat.html(opts).prop('disabled', false);
            } else {
                $bat.html('<option value="">No batches for this course</option>').prop('disabled', false);
            }
        }).fail(() => $bat.html('<option value="">-- Select Batch --</option>').prop('disabled', false));
    });

    // Batch change → show/hide payment section
    $('#enroll_batch').on('change', function() {
        $(this).val() ? $('#quickPaymentSection').slideDown(200) : $('#quickPaymentSection').slideUp(200);
    });
    ...
    $('#studentForm').on('submit', function (e) {
        ...
        const enrollBatchId  = $('#enroll_batch').val();
        ...
        const doEnroll       = !!(enrollCourseId && enrollBatchId);
        ...
        $.ajax({
            url: '../views/models/api/student_api.php?action=register',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function (res) {
                ...
                // Step 2: Enroll
                $.post('../views/models/api/enrollment_api.php?action=enroll', {
                    student_id: studentId, course_id: enrollCourseId,
                    batch_id: enrollBatchId, enrollment_date: enrollDate, status: enrollStatus
                }, function(eRes) {
                    if (!eRes.success) {
                    ...
                    // Step 3: Payment
                    $.post('../views/models/api/payment_api.php?action=record', {
    ...
    window.openEditStudent = function(id) {
        $.getJSON('../views/models/api/student_api.php?action=get&id=' + id, function(res) {
            ...
            $('#editPhotoPreview').attr('src', s.photo_url ? s.photo_url : 'https://via.placeholder.com/100?text=Photo');
    ...
    $('#editStudentForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '../views/models/api/student_api.php?action=update',
    ...
    window.deleteStudent = function(id, name) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post('../views/models/api/student_api.php?action=delete', { id: id }, function(res) {
                    if (res.status === 'success') {
