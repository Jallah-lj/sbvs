...
$photo = $baseUrl . 'assets/img/default-avatar.png'; // default
$title = '';
$name = '';
$idNumber = '';
$roleText = '';
...
function resolveIdPhotoUrl(?string $photoPath, string $baseUrl): string {
    $default = $baseUrl . 'assets/img/default-avatar.png';
    ...
    $currentAbs = $projectRoot . '/' . $relative;
    if (is_file($currentAbs)) {
        return $baseUrl . $relative;
    }

    $legacyAbs = dirname($projectRoot) . '/' . $relative;
    if (is_file($legacyAbs)) {
        $legacyBaseUrl = preg_replace('#/[^/]+/$#', '/', $baseUrl);
        return $legacyBaseUrl . $relative;
    }
...
if ($type === 'student') {
    ...
    $photo = resolveIdPhotoUrl($data['photo_url'] ?? '', $baseUrl);
} elseif ($type === 'teacher' || $type === 'staff') {
    $sessionRole = $_SESSION['role'] ?? '';
    $sessionUserId = (int)($_SESSION['user_id'] ?? 0);

    if ($sessionRole === 'Teacher') {
        $ownStmt = $db->prepare("SELECT id FROM teachers WHERE id = ? AND user_id = ? LIMIT 1");
        $ownStmt->execute([$id, $sessionUserId]);
    ...
    $stmt = $db->prepare("
        SELECT t.teacher_id, u.name, u.email, u.role, t.phone, t.specialization, t.photo_url, b.name as branch_name 
        FROM teachers t
