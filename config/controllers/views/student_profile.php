                ...
                <div class="card shadow-sm border-0 mb-3 text-center">
                    <div class="card-body py-5">
                        <div class="d-flex justify-content-center mb-4">
                            <?php
                            $baseUrl = rtrim(defined('BASE_URL') ? BASE_URL : '/sbvs/sbvs/', '/') . '/';
                            $photoRel = ltrim((string)($student['photo_url'] ?? ''), '/');
                            $projectRoot = realpath(dirname(__DIR__, 3)) ?: dirname(__DIR__, 3);
                            $photoUrl = '';

                            if ($photoRel !== '') {
                                $currentAbs = $projectRoot . '/' . $photoRel;
                                if (is_file($currentAbs)) {
                                    $photoUrl = $baseUrl . $photoRel;
                                } else {
                                    $legacyAbs = dirname($projectRoot) . '/' . $photoRel;
                                    if (is_file($legacyAbs)) {
                                        $legacyBaseUrl = preg_replace('#/[^/]+/$#', '/', $baseUrl);
                                        $photoUrl = $legacyBaseUrl . $photoRel;
                                    }
                                }
                            }
